...
//Mod variables
bool superjump = false, infiniteAmmo = false, godMode = false;
int wantedLevel = 0;
...
void update_menu() {
	...
	*/
	...
	if (menu.CurrentMenu("mainmenu")) {
		...
		menu.MenuOption("Player Options", "playerSubmenu", { "Options which affect the player." });
	...
	if(menu.CurrentMenu("playerSubmenu"))
	{
		//example:
		...
		//Other example usage: https://github.com/E66666666/GTAVStancer/blob/master/SlamIt/script.cpp#L405

		//vehicle spawn https://github.com/E66666666/GTAVAddonLoader/blob/master/GTAVAddonLoader/script.cpp#L344


		menu.Title("Player Options");
		menu.Subtitle("");

		menu.BoolOption("Infinite Ammo", infiniteAmmo, { "Enable Infinite Ammo for all weapons." });
		menu.BoolOption("Super Jump", superjump, {});
		menu.BoolOption("God Mode", godMode, {});


		if (menu.IntOption("Wanted Level", wantedLevel, 0, 5, 1, { "Increase or decrease wanted level.", "Set to 0 for no level; 5 for max." }))
		{
			pMenu.setWantedLevel(wantedLevel); //Doesn't work
		}
	}
...
void update_game() 
{
	/* Whatever happens normally in your script! - i.e things which need to be looped or checked constantly (i.e is toggle super jump)*/

	if (infiniteAmmo)
		pMenu.setPlayerInfiniteAmmo(!infiniteAmmo);

	if (superjump)
		pMenu.setPlayerSuperJump(!superjump);

	if (godMode)
		pMenu.setPlayerGodMode(playerPed, !godMode);
}
